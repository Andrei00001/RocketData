...
class EnterpriseView(views.APIView):
    ...
    def get(self, request, *args, **kwargs):
        return Response(get_data_for_enterprise(list(Enterprise.objects.filter(user__user=request.user))))
...
class EnterpriseFromCountryView(views.APIView):
    ...
    def get(self, request, country, *args, **kwargs):
        return Response(get_data_for_enterprise(
            list(Enterprise.objects.filter(user__user=request.user, contacts__country=country).all())))
    ...
    def get(self, request, *args, **kwargs):
        ...
        return Response(
            get_data_for_enterprise(
                list(Enterprise.objects.filter(user__user=request.user, recipient__price__gt=average_debt).all())))
...
class ProductsEnterpriseView(views.APIView):
    ...
    def get(self, request, product_id: int, *args, **kwargs):
        return Response(
            get_data_for_enterprise(
                list(Enterprise.objects.filter(user__user=request.user, products__products=product_id).all())))
...
def get_data_for_enterprise(enterprises: list[Enterprise]) -> list[list[dict]]:
    ...
    for enterprise in enterprises:
        ...
        products_data = ProductsSerializer(Products.objects.filter(enterprise__enterprise=enterprise),
                                           many=True)
        employees_data = EnterpriseEmployeesSerializer(User.objects.filter(enterprise_user__enterprise=enterprise),
                                                       many=True)
